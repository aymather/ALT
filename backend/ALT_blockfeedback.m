function ALT_blockfeedback(settings,trialseq,it)
...
succstop = blocktrials(blocktrials(:,id.acc)==3,:);
failstop = blocktrials(blocktrials(:,id.acc)==4,:);
stopsucc = round(100*size(succstop,1) / (size(failstop,1)+size(succstop,1)));
...
if it < size(trialseq,1)
    ALT_intro(settings, trialseq, it);
end