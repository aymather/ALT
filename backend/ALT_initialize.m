function settings = ALT_initialize(data)
...
load(fullfile(fileparts(which('ALT.m')), 'backend', 'novelsounds.mat'));
settings.sound.novelsounds = novelsounds(randperm(length(novelsounds))); % randomize sounds
PsychPortAudio('FillBuffer', settings.sound.audiohandle, settings.sound.novelsounds{1,1}'); % buffer sound
...
owd = settings.screen.outwindowdims;
    settings.coordinates = [owd(3)/2 owd(4)/2-settings.layout.size.diamond; owd(3)/2+settings.layout.size.diamond owd(4)/2; owd(3)/2 owd(4)/2+settings.layout.size.diamond; owd(3)/2-settings.layout.size.diamond owd(4)/2];
