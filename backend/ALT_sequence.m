function trialseq = ALT_sequence(settings)
...
function kldiv = surprise(invector)

    % preassign
    crits = find(invector==1);
    nt = size(crits,1);
    prior = zeros(nt,1);
    posterior = prior; kldiv = prior;

    % go through
    for in = 1:nt
        % prior
        prior(in) = (in-1)/(crits(in)-1);
        % posterior
        posterior(in) = in/crits(in);
        % KL divergence
        kldiv(in) = log2(posterior(in)/prior(in));
    end
    kldiv(1) = 1;
